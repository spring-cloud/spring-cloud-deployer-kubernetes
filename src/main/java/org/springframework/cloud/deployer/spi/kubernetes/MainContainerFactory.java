...
public class MainContainerFactory extends AbstractContainerFactory {
	...
	public Container create(String appId, AppDeploymentRequest request, Integer port, Integer instanceIndex,
		boolean hostNetwork) {
		String image = resolveImageName(request.getResource());
		...
		container.withName(appInstanceId).withImage(image).withEnv(envVars).withArgs(appArgs)
		...
		if (port != null) {
			...
			container.withReadinessProbe(
				new MainProbeCreator(port, properties.getReadinessProbePath(), properties.getReadinessProbeTimeout(),
				...
				new MainProbeCreator(port, properties.getLivenessProbePath(), properties.getLivenessProbeTimeout(),
	 ...
	 */
	private static class MainProbeCreator {
		...
		MainProbeCreator(Integer externalPort, String endpoint, int timeout, int initialDelay, int period, String prefix,
		...
		Probe create() {
			return new KubernetesProbeBuilder(externalPort, KubernetesDeployerProperties.Probe.HandlerType.http)
